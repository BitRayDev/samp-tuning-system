...
#define MAX_VEHICLE_STYLING_OBJECTS 8
...
enum E_VEHICLE
{
    ...
    vehicle_styling_objects[MAX_VEHICLE_STYLING_OBJECTS]
}
new VehicleInfo[MAX_VEHICLES][E_VEHICLE];

enum E_STYLING_OBJECTS
{
	styling_object_vehicle_model,
	styling_object_id,
	styling_object_slot_id,
	styling_object_name[16],
	styling_object_price
}
/*AttachObjectToVehicle(objectid, vehicleid, 0.000000, -1.959998, 0.279999, 0.000000, 0.000000, 0.000000); //Object Model: 1000 |  bullet_spoiler_pro
AttachObjectToVehicle(objectid, vehicleid, 0.000000, -2.014998, 0.274999, 0.000000, 0.000000, 0.000000); //Object Model: 1015 |  bullet_race_spoiler
AttachObjectToVehicle(objectid, vehicleid, 0.000000, -2.044999, 0.289999, 0.000000, 0.000000, 0.000000); //Object Model: 1049 |  bullet_alien_spoiler
AttachObjectToVehicle(objectid, vehicleid, 0.000000, 0.000000, 0.639999, 0.000000, 0.000000, 0.000000); //Object Model: 11701 |  bullet_police_light1
AttachObjectToVehicle(objectid, vehicleid, -0.914999, 0.000000, -0.444999, 0.000000, 0.000000, 0.000000); //Object Model: 18648 |  bullet_neon
AttachObjectToVehicle(objectid, vehicleid, 0.894999, 0.000000, -0.444999, 0.000000, 0.000000, 0.000000); //Object Model: 18648 |  bullet_neon
AttachObjectToVehicle(objectid, vehicleid, 0.000000, 0.219999, 0.649999, 0.000000, 0.000000, 0.000000); //Object Model: 18646 |  bullet_police_light_small
AttachObjectToVehicle(objectid, vehicleid, 0.009999, 2.355006, 0.000000, 0.000000, 0.000000, 0.000000); //Object Model: 1254 |  bullet_cherep
AttachObjectToVehicle(objectid, vehicleid, 0.000000, -1.439998, 0.799999, 0.000000, 0.000000, 0.000000); //Object Model: 2614 |  bullet_flags
AttachObjectToVehicle(objectid, vehicleid, 0.044000, -1.401501, 0.799999, 0.000000, 0.000000, 179.995651); //Object Model: 2614 |  bullet_flags*/
new StylingObjects[][E_STYLING_OBJECTS]
{
	{0, 0, 0, "", 0},
	{541, 11701, 0, "Police Light", 25000},
	{541, 18648, 1, "Neon", 50000},
	{541, 1254, 2, "Skull Hood", 12000},
	{541, 2614, 3, "Flags", 25000}
}

stock InstallStylingObject(vehicleid, objectid)
{
	if(StylingObjects[objectid][styling_object_vehicle_model] != GetVehicleModel(vehicleid)) return 0;
...
function SecondTimer()
{
	foreach(new i:Player)
	{
		...
		{
		    if(!IsPlayerInRangeOfPoint(i, 200.0, 430.8073,-869.9299,2735.6948))
		    {
		        TuningMechanicEndJob(i);
		    }
		}
		if(TuningCustomerInfo[tuning_customer_id] == i)
		{
		    if(!IsPlayerInRangeOfPoint(i, 200.0, 430.8073,-869.9299,2735.6948))
		    {
				foreach(new e:Player)
				{
				    if(GetPVarInt(e, "TuneMechanic:Active"))
					{
					    TogglePlayerControllable(e, 1);
					    RemovePlayerAttachedObject(e, 0);
					    ClearAnimations(e);
					    
					    SendClientMessage(e, COLOR_YELLOW, "[Tuning]: Заказчик вышел из гаража. Все неустановленные заказы анулированны");
					}
				}
				
				GetPlayerOutOfTuning(i);
			}
		}
	}
	for(new i; i<MAX_TUNING_ORDERS; i++)
	{
	    ...
	    if(TuningOrders[i][tuning_order_install_progress] > 0)
	    {
            ...
            if(TuningOrders[i][tuning_order_install_progress] <= 0)
            {
                ...
                VehicleInfo[vehicle_id][vehicle_tuning_elements][TuningElements[element_id][tuning_element_categorie_id]] = element_id;
				...
				{
				    GetPlayerOutOfTuning(customer_id);
...
public OnGameModeInit()
{
	...
	Create3DTextLabel("Тюнинг\nПосигнальте ('H')", 0xFFFFFFFF, 979.3741,-1256.5996,16.9535, 25.0, 0, 1);
	
	CreatePickup(1318, 23, 986.9020,-1251.3250,16.9844, -1);
	Create3DTextLabel("Вход в гараж\nНажмите 'ALT'", 0xFFFFFFFF, 986.9020,-1251.3250,16.9844, 10.0, 0, 1);
	
	CreatePickup(1318, 23, 416.8541,-868.3005,2735.6948, -1);
	Create3DTextLabel("Нажмите 'ALT'", 0xFFFFFFFF, 416.8541,-868.3005,2735.6948, 10.0, 0, 1);
	...
	AddPlayerClass(0, 979.7585,-1273.0344,15.1064, 269.1425, 0, 0, 0, 0, 0, 0);
	return 1;
}
...
public OnPlayerDisconnect(playerid, reason)
{
	if(TuningCustomerInfo[tuning_customer_id] == playerid)
	{
		...
		for(new i; i<MAX_TUNING_ORDERS; i++)
		{
		    ...
		    if(TuningOrders[i][tuning_order_install_progress])
		    {
		        TogglePlayerControllable(mechanic_id, 1);
		        TempInfo[mechanic_id][temp_tuning_order_id] = INVALID_TUNING_ORDER;
                RemovePlayerAttachedObject(mechanic_id, 0);
				ClearAnimations(mechanic_id);
		    }
	...
	if(GetPVarInt(playerid, "TuneMechanic:Active"))
	{
		...
		if(!mechanics)
		{
            ...
            PutPlayerInVehicle(customer_id, vehicle_id, 0);
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(PRESSED(KEY_WALK))
	{
	    if(IsPlayerInRangeOfPoint(playerid, 1.5, 986.9020,-1251.3250,16.9844))
	    {
	        SetPlayerPos(playerid, 419.0073,-868.4658,2735.6948);
	        SetPlayerFacingAngle(playerid, 270.0);
	        SetCameraBehindPlayer(playerid);
	    }
        if(IsPlayerInRangeOfPoint(playerid, 1.5, 416.8541,-868.3005,2735.6948))
        {
            if(GetPVarInt(playerid, "TuneMechanic:Active"))
            {
				return ShowPlayerDialog(playerid, dTuneMechanic+2, DIALOG_STYLE_MSGBOX, "Желаете выйти из гаража?", "Если вы покинете гараж, вы будете\nавтоматически уволены с работы тюнинг-механика.\n\nВыйти из гаража?", "Да", "Нет");
            }
            if(TuningCustomerInfo[tuning_customer_id] == playerid)
            {
				return ShowPlayerDialog(playerid, dTuneMechanic+3, DIALOG_STYLE_MSGBOX, "Желаете выйти из гаража?", "Если вы покинете гараж, ваш заказ будет\nавтоматически аннулирован.\n\nВыйти из гаража?", "Да", "Нет");
            }

            SetPlayerPos(playerid, 986.5435,-1255.5505,16.9844);
		...
		if(IsPlayerInRangeOfPoint(playerid, 1.5, 434.8695,-864.2725,2739.4128))
		{
		    if(GetPVarInt(playerid, "TuneMechanic:Active"))
		        ...
		        //ShowPlayerDialog(playerid, dTuneMechanic, DIALOG_STYLE_MSGBOX, "Тюнинг-механик | ",
				//"Устроиться\nСтатистика\nИнформация о работеВы желаете устроиться на работу тюнинг-механика?", "Да", "Нет");
		        return ShowPlayerDialog(playerid, dTuneMechanic, DIALOG_STYLE_MSGBOX, "Работа", "Вы желаете устроиться на работу тюнинг-механика?", "Да", "Нет");
		}
		if(GetPVarInt(playerid, "TuneMechanic:Active"))
  		{
			...
			if(IsPlayerInRangeOfPoint(playerid, 1.5, 502.6337,-865.2659,2738.5400))
			{
			    if(TempInfo[playerid][temp_tuning_order_id] == INVALID_TUNING_ORDER)
			    {
			        for(new i; i<MAX_TUNING_ORDERS; i++)
				    {
						...
						format:g_str_small("[Tuning]: Вы взяли {ffffff}'%s %s'{33AA33}. Теперь установите его на автомобиль.", TuningCategories[TuningElements[element_id][tuning_element_categorie_id]][tuning_categorie_name_rus], TuningElements[element_id][tuning_element_name]);
	...
	if(PRESSED(KEY_CROUCH))
	{
	    ...
	    {
	        if(IsPlayerInRangeOfPoint(playerid, 7.5, 979.3741,-1256.5996,16.9535))
	        {
		        if(TuningCustomerInfo[tuning_customer_id] == -1)
		        {
		        	PutPlayerInTuning(playerid);
		        }
		        else return SendClientMessage(playerid, COLOR_RED, "[Tuning]: На данный момент гараж занят. Дождитесь свой очереди.");
	        }
	    }
	}
	if(PRESSED(KEY_FIRE))
	    ...
	    if(IsPlayerInAnyVehicle(playerid) )
	    {
	        ...
	        if(GetPVarInt(playerid, "Tuning:Active") == 2)
	        {
				...
				for(new i; i<9; i++) if(i<sizeof(TuningCategories))
				{
				    ...
				    if(TempInfo[playerid][temp_selected_categories][i] == -1) continue;
				    if(TempInfo[playerid][temp_selected_categories][i] == TempInfo[playerid][temp_selected_categorie])
				    {
				        TempInfo[playerid][temp_selected_td_item] = i;
				    }
	...
	if(PRESSED(KEY_SECONDARY_ATTACK))
	{
	    if(IsPlayerInAnyVehicle(playerid))
	    {
			...
			else if(GetPVarInt(playerid, "Tuning:Active") == 2)
			{
				...
				new
					item = TempInfo[playerid][temp_selected_items][TempInfo[playerid][temp_selected_td_item]],
				...
				else
				{
					...
					{
					    format:g_str_least("%d$", TuningElements[TempInfo[playerid][temp_selected_items][i]][tuning_element_price]);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
	    ...
	    case dTuneMechanic+1:
	    {
			...
			TuningMechanicEndJob(playerid);
	    }
	    case dTuneMechanic+2:
	    {
	        if(!response) return 1;
...
{
	if(TuningElements[VehicleInfo[vehicleid][vehicle_tuning_elements][TUNING_CATEGORIE_TIRES]][tuning_element_id] == 1200)
	{
		new
		    panels,
			doors,
...
stock TuningMechanicEndJob(playerid)
{
    SetPlayerSkin(playerid, 0);
	SetPVarInt(playerid, "TuneMechanic:Active", 0);
	
	new order = TempInfo[playerid][temp_tuning_order_id];
	
	if(order != INVALID_TUNING_ORDER)
	{
	    if(TuningOrders[order][tuning_order_install_progress])
	    {
	        TogglePlayerControllable(playerid, 1);
	        ClearAnimations(playerid);
	        RemovePlayerAttachedObject(playerid, 0);
	    }
        TuningOrders[order][tuning_order_mechanic_id] = -1;
...
stock TuningMechanicRise(playerid)
{
    ...
    SendClientMessage(playerid, COLOR_GREEN, g_str_small);
    format:g_str_small("[TUNING]: Для следующего повышения нужно выполнить {ffffff}%d {33aa33}заказов!", TuningMechanicLevels[uInfo[playerid][uTuningMechanicLevel]][tuning_mechanic_orders_needed]-uInfo[playerid][uTuningMechanicOrders]);
    SendClientMessage(playerid, COLOR_GREEN, g_str_small);
	return 1;
}

stock GetPlayerOutOfTuning(playerid)
{
    new vehicle_id = TuningCustomerInfo[tuning_vehicle_id];
...
stock UpdateElementsList(playerid)
{
	...
	if(TempInfo[playerid][temp_selected_td_item] >= array_size)
	{
	    ...
	    if(next_element)
		{
			...
			{
			    if(i+1 < array_size)
                	TempInfo[playerid][temp_selected_items][i] = TempInfo[playerid][temp_selected_items][i+1];
				else
				    TempInfo[playerid][temp_selected_items][i] = next_element;
				PlayerTextDrawSetString(playerid, TuningItemTD[playerid][i][0], TuningElements[TempInfo[playerid][temp_selected_items][i]][tuning_element_name]);

				
				if(TempInfo[playerid][temp_selected_tuning_elements][categorie] == TempInfo[playerid][temp_selected_items][i])
					g_str_least = "X";
				else
					format:g_str_least("%d$", TuningElements[TempInfo[playerid][temp_selected_items][i]][tuning_element_price]);
		    ...
		    if(previous_element)
			{
			    for(new i=array_size-1; i>=0; i--)
				{
				    if(i-1 >= 0)
					    ...
					    TempInfo[playerid][temp_selected_items][i] = previous_element;
			...
			else
			{
			    ...
			    for(new i = sizeof(TuningElements)-1; i >= 0; i--) if(td >= 0)
			    {
			        if(TuningElements[i][tuning_element_categorie_id] == categorie)
			        {
       					...
       					{
                            TempInfo[playerid][temp_selected_items][td] = i;
                            
                            PlayerTextDrawSetString(playerid, TuningItemTD[playerid][td][0], TuningElements[i][tuning_element_name]);
                            
							if(TempInfo[playerid][temp_selected_tuning_elements][categorie] == TempInfo[playerid][temp_selected_items][td])
								g_str_least = "X";
							else
	...
	for(new i; i<9; i++) if(i<sizeof(TuningElements))
	{
	    while(c+1 < sizeof(TuningElements))
	    {
	        ...
	        if(TuningElements[c][tuning_element_categorie_id] == categorieID)
			{
	            ...
	            {
	                TempInfo[playerid][temp_selected_items_array_size]++;
			        TempInfo[playerid][temp_selected_items][i] = c;
...
stock ShowTuningCategories(playerid) //просто показывает текстдравы игроку
{
	...
	for(new i; i<9; i++)
	    TempInfo[playerid][temp_selected_categories][i] = -1;
...
{
	if(categorieid == TUNING_CATEGORIE_STYLING)
	{
	    switch(vehicleid)
	    {
	        case 541: return 1;
...
stock FindNextElement(playerid)
{
	new
	    selected_item = TempInfo[playerid][temp_selected_td_item],
		vehicleid = GetPlayerVehicleID(playerid),
		current_max_element_id = TempInfo[playerid][temp_selected_items][selected_item]+1,
...
stock FindPreviousElement(playerid)
{
	new
		vehicleid = GetPlayerVehicleID(playerid),
		current_min_element_id = TempInfo[playerid][temp_selected_items][0]-1,
		categorieid =  TempInfo[playerid][temp_selected_categorie];
...
CMD:tp(playerid)
{
	SetPlayerPos(playerid, 437.8956,-862.7302,2739.4128);
	return 1;
}
